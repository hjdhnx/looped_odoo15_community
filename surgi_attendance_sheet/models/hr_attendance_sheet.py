...
class AttendanceSheet(models.Model):
    ...
    def action_auto_attendance(self):
        for rec in self:
            rec.get_attendances(True)
            # return
        ...
        for rec in self:
            if rec.state != 'confirm':
                raise ValidationError(_(
                        "Sorry!!,Attendance must be in Confirm State to be Approved ")+str(rec.name))
            
            rec.action_create_penalties()
        ...
        for sheet in self:
            ...
            for lateline in late_lines:
                values = {
                    ...
                    'act_hours':lateline.act_late_in,
                }
            ...
            for abline in absence_lines:
                values = {
                    ...
                    'act_hours':abline.act_diff_time,
                }
                penalty_obj.create(values)
            ...
            for diffline in diff:
                values = {
                    ...
                    'act_hours':diffline.act_diff_time,
                }
                penalty_obj.create(values)
    ...
    def _get_emp_exceuse_intervals(self, emp, start_datetime=None,
                                 end_datetime=None):
        

        lates = []
        late_obj = self.env['late.request']
        late_ids = late_obj.search([
            ('employee_id', '=', emp.id),
            ('state', '=', 'approve'),
            ('is_business_trip','=',True)])

        for late in late_ids:
            date_from = late.date_from
            if end_datetime and date_from > end_datetime:
                continue
            date_to = late.date_to
            if start_datetime and date_to < start_datetime:
                continue
            lates.append((date_from, date_to))
        return lates
    ...
    def get_attendances(self,auto_attendance=False):
        for att_sheet in self:
            ...
            days = att_sheet.env['hr.attendance.policy'].last_day_of_month(to_date)
            
            
            month_number_of_days = att_sheet.number_of_days_in_month(to_date.year,to_date.month)
            ...
            late_cnt = []
            diff_cnt = []
            # print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>",all_dates,from_date,to_date)
            for day in all_dates:
                ...
                if contract.multi_shift:
                    work_intervals = att_sheet.employee_id.get_employee_shifts(
                        day_start,
                        day_end, tz)
                    # print(">>>>>>>>>>>>>>>>woek interval",work_intervals)
                ...
                lates = self._get_emp_exceuse_intervals(emp, day_start, day_end)
                            ...
                            for i, work_interval in enumerate(work_intervals):
                                ...
                                if miss_att_intervals and miss_att_intervals[0][
                                    ...
                                    policy_miss_late, late_cnt = policy_id.get_late(
                                        float_miss_late, late_cnt,month_days=month_number_of_days)
                                    ...
                                    policy_miss_diff, diff_cnt = policy_id.get_diff(
                                        float_miss_diff, diff_cnt,month_days=month_number_of_days)
                        ...
                        for i, work_interval in enumerate(work_intervals):
                            ...
                            if diff_intervals:
                                for diff_in in diff_intervals:
                                    ...
                                    #incase business trip
                                    if lates:
                                        status = "late" #business trip
                                        diff_clean_intervals = calendar_id.att_interval_without_leaves(
                                            diff_in, lates)
                                        for diff_clean in diff_clean_intervals:
                                            diff_time += diff_clean[1] - \
                            ...
                            policy_late, late_cnt = policy_id.get_late(
                                float_late, late_cnt,contract,month_days=month_number_of_days)
                            ...
                            if status == 'ab':
                                ...
                                float_diff = policy_id.get_absence(float_diff,
                                                                   abs_cnt,contract,month_days=month_number_of_days)
                            else:
                                act_float_diff = float_diff
                                float_diff, diff_cnt = policy_id.get_diff(
                                    float_diff, diff_cnt,contract,month_days=month_number_of_days)
                            if not auto_attendance:
                                values = {
                                    'date': date,
                                    'day': day_str,
                                ...
                                if status not in ['leave','weekend','late']:
                                    status = False
                                values = {
                                    'date': date,
                                    'day': day_str,
                                    ...
                                    # 'late_in': policy_late,
                                    # 'act_late_in': act_float_late,
                                    'worked_hours': pl_sign_out - pl_sign_in ,#float_worked_hours,
                                    # 'overtime': float_overtime,
                                    # 'act_overtime': act_float_overtime,
                                    # 'diff_time': float_diff,
                                    # 'act_diff_time': act_float_diff,
