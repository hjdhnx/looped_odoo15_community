...
class HrAttendancePolicy(models.Model):
    ...
    overtime_based_on = fields.Selection([
        ('basic', 'Basic Salary'),
        
        ('total', 'Total salary'),
    ], string='Based on',default="basic")
    overtime_month_days = fields.Integer(default=30)
    #check in new databaase to test if it pass and set value or not
    overtime_rule = fields.Many2one('hr.salary.rule',
    default=lambda self:self.env.ref('rm_hr_attendance_sheet.hr_salary_rule_att_overtime').id)
    overtime_hour_rate = fields.Float(default=1.5)
    ...
    def get_absence(self, period, cnt,contract_id=False,month_days=1):
        res = period
        flag = False
        if self:
            if self.absence_rule_id:
                ...
                for ln in abs_ids:
                    if cnt >= int(ln.counter):
                        ...
                        amount = contract_id.get_salary_based_on(self.absence_rule_id.deduction_based_on)
                        res = ln.rate * (amount/month_days)
...
class HrOvertimeRule(models.Model):
    ...
    deduction_based_on = fields.Selection([
        ...
        ('total', 'Total salary'),
    ], string='Deduction based on',default='basic')
...
class HrLateRule(models.Model):
    ...
    month_days = fields.Integer(default=30)
    rule_id = fields.Many2one('hr.salary.rule',
    default=lambda self:self.env.ref('rm_hr_attendance_sheet.hr_salary_rule_att_late').id)
    
    @api.onchange('deduction_based_on','overtime_month_days')
    def onchange_based_on_set_rule(self):
        for rec in self:
            result = 'result=-worked_days.ATTSHLI.number_of_hours * contract.wage / (9*26)'

            if rec.deduction_based_on == 'basic':
                # result = 'result=-(categories.BASIC - worked_days.ATTSHLI.number_of_hours)'
                result = 'result=-(worked_days.ATTSHLI.number_of_hours)'
            elif rec.deduction_based_on == 'total':
                # result = 'result=-(categories.NET - worked_days.ATTSHLI.number_of_hours)'
                result = 'result=-(worked_days.ATTSHLI.number_of_hours)'
            if rec.rule_id:
                rec.rule_id.amount_python_compute = result
            # 1/0
...
class HrDiffRule(models.Model):
    ...
    month_days = fields.Integer(default=30)
    rule_id = fields.Many2one('hr.salary.rule',
    default=lambda self:self.env.ref('rm_hr_attendance_sheet.hr_salary_rule_att_diff').id)
...
class HrAbsenceRule(models.Model):
    ...
    month_days = fields.Integer(default=30)
    rule_id = fields.Many2one('hr.salary.rule',
    default=lambda self:self.env.ref('rm_hr_attendance_sheet.hr_salary_rule_att_absence').id)
