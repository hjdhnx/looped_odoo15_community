...
class SearchResult(http.Controller):
    ...
    def load_my_map(self , **post):
        ...
        for attendance_id in attendance_ids:
            ...
            ch_dt = pytz.utc.localize(datetime.strptime(str(attendance_id.check_in), DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local)
    ...
    def load_attendance(self , **post):
        ...
        if 'emp' in post:
            ...
            new_tree_view = request.env.ref("emp_attendance_google_map_app.new_hr_attendance_view_tree")
            for attendance_id in attendance_ids:
                user_tz = request.env.user.tz or pytz.utc
                local = pytz.timezone(user_tz)
                ch_dt = pytz.utc.localize(datetime.strptime(str(attendance_id.check_in), DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local)
