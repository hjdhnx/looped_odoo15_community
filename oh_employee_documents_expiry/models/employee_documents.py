...
class HrEmployeeDocument(models.Model):
    ...
    notification_by = fields.Selection([
        ('email', 'By Email'),
        ('in', 'In the System'),
    ...
    @api.onchange('notification_by')
    def onchange_set_notification_type(self):
        for rec in self:
            if rec.notification_by == 'in':
                rec.notification_type = False

    def action_set_notif(self):
        for rec in self:
            if rec.notification_by == 'in':
                rec.activity_schedule(
                    'oh_employee_documents_expiry.mail_activity_emp_document_expiary',
                    note=_(
                        'Document Expiry Payment'
                    )
                    ,
                    date_deadline = rec.expiry_date,
                )

    @api.onchange('name','document_type','issue_date')
    def onchange_set_full_code(self):
        for rec in self:
            full_code = ''
            if rec.document_type:
                full_code += str(rec.document_type.name)+'/'
            if rec.issue_date:
                full_code += str(rec.issue_date.year)+'-'+str(rec.issue_date.month)+'/'
            if rec.name:
                full_code += str(rec.name)
            rec.full_code = full_code


    def write(self,vals):
        res = super(HrEmployeeDocument,self).write(vals)
        if vals.get('expiry_date',False) or vals.get('notification_by',False):
            for rec in self:
