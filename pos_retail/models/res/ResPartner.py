...
class res_partner(models.Model):
    ...
    pos_loyalty_return_point = fields.Float(
        digits=(16, 4),
        # compute="_get_point",
        string='Loyalty Return Points Total',
        help='Total point of customer can use reward program of pos system')
    pos_loyalty_net_point = fields.Float(
        digits=(16, 4),
        # compute="_get_point",
        string='Loyalty Net Points',
        help='Total point of customer can use reward program of pos system')
    ...
    group_ids = fields.Many2many(
        ...
        'group_id',
        string='Groups Name')
    mobile_ref = fields.Char('Mobile Ref')
    other_street = fields.Char()
    other_street2 = fields.Char()
    other_zip = fields.Char(change_default=True)
    other_city = fields.Char()
    other_state_id = fields.Many2one("res.country.state", string='State', ondelete='restrict', domain="[('country_id', '=?', country_id)]")
    other_country_id = fields.Many2one('res.country', string='Country', ondelete='restrict')    
    other_address_ids = fields.One2many('other.address', 'partner_id', string='')
    wallet_balance = fields.Float(readonly=1)
    # def _get_point(self):
    #     for partner in self:
    #         partner.pos_loyalty_return_point = rec.pos_loyalty_net_point = 0
