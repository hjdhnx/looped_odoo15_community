...
class pos_gift_coupon(models.Model):
	...
	def print_report_coupons(self):
		return self.env.ref('pos_retail.action_gift_coupons').report_action(self)
	...
	product_id  = fields.Many2one('product.product', domain = [('type', '=', 'service'),
		('available_in_pos', '=', True),('product_type', '=', 'is_discount')],string='Product')
	...
	applied_app = fields.Selection([('pos', 'POS'),('delivery', 'Delivery')], default='pos')
	applied_pos_ids = fields.Many2many('pos.config')
