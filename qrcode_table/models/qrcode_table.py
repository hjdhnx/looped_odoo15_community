    ...
    def _compute_barcode_url(self):
        ...
        for record in self:
            ...
            if record.floor_id and record.floor_id.pos_config_id:
                if not record.floor_id.domain_name:
                    if dom_name:
                        domain_nm = dom_name
                    else:
                        domain_nm = 'localhost:8069'
                else:
                    domain_nm = record.floor_id.domain_name
                record.barcode_url = "%s/table/%s" % (domain_nm, slug(record._origin))
...
class TableOrder(models.Model):
    ...
    state = fields.Selection([('draft', 'New'), ('in_progress', 'In Progress'), ('done', 'Done'), ('refuse', 'Refuse')], copy=False, default='draft')
