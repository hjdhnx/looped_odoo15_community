...
class StockRequestOrder(models.Model):
    ...
    def _compute_picking_ids(self):
        for record in self:
            ...
            picking_ids = []
            for pick in record.stock_request_ids.mapped("picking_ids"):
                picking_ids.append(pick.id)
            stock_mv_lines = self.env['stock.move.line'].search([('stock_request_order_id','=',self.id)])    
            
            for mv in stock_mv_lines:
                if mv.picking_id:
                    picking_ids.append(mv.picking_id.id)
            if len(picking_ids) > 0:
                record.picking_ids = picking_ids #record.stock_request_ids.mapped("picking_ids")
                
                record.picking_count = len(picking_ids)#len(record.picking_ids)
    ...
    def action_confirm(self):
        self.mapped("stock_request_ids").with_context(stock_request_order_id=self.id).action_confirm()
